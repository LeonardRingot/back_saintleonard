  ...
  describe('ParcoursService', () => {
    ...
    describe('findAll', () => {
        it('should return an array of parcours', async () => {
            ...
            jest.spyOn(parcoursRepository, 'findAll').mockResolvedValue(expectedParcours);
    ...
    describe('findById', () => {
        it('should return a parcours by its id', async () => {
          const id = 1;
          const expectedParcours: ParcoursDto = { id_parcours: id, name: 'Parcours 1' };
          jest.spyOn(parcoursRepository, 'findById').mockResolvedValue(expectedParcours);
      ...
      describe('create', () => {
        it('should create a new parcours', async () => {
          const parcoursDto: ParcoursDto = {
            ...
            points: [
              { id_point: 1, name:'test1',position: 1, mainDescription:'oe',smallDescription:'a', lon:63, lat:6, QrCode:"ae"},
              { id_point: 2, name:'test2', position: 2 ,mainDescription:'oe',smallDescription:'a', lon:63, lat:6, QrCode:"ae"},
              { id_point: 3, name:'test3', position: 3 ,mainDescription:'oe',smallDescription:'a', lon:63, lat:6, QrCode:"ae"}
            ]
          };
          ...
          Parcours.create = jest.fn().mockResolvedValue(createdParcours);
           (ParcoursPoint as any).create.mockResolvedValue(null);
          (sequelize.transaction as jest.Mock).mockResolvedValue({ commit: jest.fn(), rollback: jest.fn() });
          ...
          expect(Parcours.create).toHaveBeenCalledWith(
            {
              ...
              id_animation: parcoursDto.animation ? parcoursDto.animation.id_animation : null,
            },
